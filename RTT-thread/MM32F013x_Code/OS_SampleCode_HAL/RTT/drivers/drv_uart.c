...
unsigned char uart1_buff[40];
int len,lock_flag,unlock_flag; 
void uart_receive(uint16_t buf)
{
     rt_base_t level;
...
void sned_buff(unsigned char *p,int len)
{
    int i;
    for(i=0;i<len;i++){
      while ((UART1->CSR & UART_CSR_TXC) == 0);
      WRITE_REG(UART1->TDR, (*p & 0xFFU));
    // while()
        p++;
    }
